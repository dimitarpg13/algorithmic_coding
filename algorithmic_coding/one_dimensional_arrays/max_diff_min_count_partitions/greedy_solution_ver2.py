            ...
            if i == 0:
                ...
                if idx < len(self.seq_info) and idx >= 0:
                    ...
                    if found_seq_info[self.MIN_VAL] <= val <= found_seq_info[self.MAX_VAL]:
                        ...
                        print(f"In k-elif: idx={idx}, val={val}, seqs = {self.seqs}, seq_info={self.seq_info}")
                    ...
                    # we did not find a sequence that can absorb the new value
                        ...
                        # idx is negative, we did not find a sequence that can absorb the new value
                        prev_seq = cur_seq
                        prev_seq_info = cur_seq_info
                        cur_seq = list()
                        cur_seq.append(val)
                        ...
                        cur_seq_info[self.MIN_IDX] = i
                        cur_seq_info[self.MAX_IDX] = i
                        self.seqs.insert(0, cur_seq)
                        self.seq_info.insert(0, cur_seq_info)
